...
constexpr auto NO_COLOR = -1;
constexpr auto RESIZE_FACTOR = 20;
constexpr auto MATLAB_INDEX_FACTOR = 1;
...
namespace p1d
{
	...
	*/
	...
	struct TComponent
	{
		...
		T MinValue; //redundant, but makes life easier
	...
	*/
	...
	public:
		...
		*/
		void PrintPairs(const std::vector<TPairedExtrema<T>>& pairs) const
		{
		...
		*/
		void PrintResults(const T threshold = 0.0, const bool matlabIndexing = false) const
		{
			if (threshold < 0)
			{
			...
			{
				std::vector<TPairedExtrema<T>> pairs;
		...
		*/
		bool GetPairedExtrema(std::vector<TPairedExtrema<T>>& pairs, const T threshold = 0, const bool matlabIndexing = false) const
		{
			...
			pairs = std::vector<TPairedExtrema<T>>(lower_bound, PairedExtrema.end());
	...
	*/
		bool GetExtremaIndices(std::vector<int>& min, std::vector<int>& max, const T threshold = 0, const bool matlabIndexing = false) const
		{
	...
	protected:
		...
		*/
		std::vector<TIdxAndData<T>> SortedData;
		...
		*/
		std::vector<TComponent<T>> Components;


		/*!
			A vector of paired extrema features - always a minimum and a maximum.
		*/
		std::vector<TPairedExtrema<T>> PairedExtrema;
		...
		*/
		void CreateComponent(const int minIdx)
		{
			...
			{
				Components.reserve(2ull * TotalComponents + 1);
			}
		...
		*/
		void Watershed()
		{
			...
			for (auto p = SortedData.begin(); p != SortedData.end(); p++)
			{
				int i = (*p).Idx;
				...
				if (i == 0)
				{
					if (Colors[i + 1ull] == NO_COLOR)
					{
						CreateComponent(i);
					}
					else
					{
						ExtendComponent(Colors[i + 1ull], i);  //in this case, local max as well
					}
				...
				else if (i == Colors.size() - 1) //right most vertex - look only to the left
				{
					...
					{
						ExtendComponent(Colors[i - 1ull], i);
					}
					continue;
				}

				//look left and right
				if (Colors[i - 1ull] == NO_COLOR && Colors[i + 1ull] == NO_COLOR) //local minimum - create new component
				{
					CreateComponent(i);
				}
				else if (Colors[i - 1ull] != NO_COLOR && Colors[i + 1ull] == NO_COLOR) //single neighbor on the left - extnd
				{
					ExtendComponent(Colors[i - 1], i);
				}
				else if (Colors[i - 1ull] == NO_COLOR && Colors[i + 1ull] != NO_COLOR) //single component on the right - extend
				{
					ExtendComponent(Colors[i + 1ull], i);
				}
				else if (Colors[i - 1ull] != NO_COLOR && Colors[i + 1ull] != NO_COLOR) //local maximum - merge components
				{
					int leftComp, rightComp;

					leftComp = Colors[i - 1ull];
					rightComp = Colors[i + 1ull];
					...
					Colors[i] = Colors[i - 1ull]; //color should be correct at both sides at this point
				}
			}
		}
		...
		*/
		auto FilterByPersistence(const T threshold = 0) const
		{
			if (threshold == 0 || threshold < 0) return PairedExtrema.begin();

			TPairedExtrema<T> searchPair{};
